...
- **用户认证**：登录、登出、登录状态检测
- **用户管理**：注册、增删改查、状态变更、批量查询、独立修改密码
- **角色管理**：角色增删改查、角色分配、角色解绑、角色目录查询
...
| 5 | 获取指定用户信息 | `/auth/user/{userId}` | GET | 本人或管理员 |
| 6 | 用户注册 | `/auth/user/register` | POST | 无 |
| 7 | 添加用户 | `/auth/user/add` | POST | 管理员 |
| 8 | 更新用户信息 | `/auth/user/update` | POST | 本人或管理员 |
| 9 | 修改密码 | `/auth/user/changePassword` | POST | 登录用户 |
| 10 | 删除用户 | `/auth/user/delete` | POST | 管理员 |
| 11 | 修改用户状态 | `/auth/user/changeStatus` | POST | 管理员 |
| 12 | 获取用户列表 | `/auth/user/list` | GET | 管理员 |
| 13 | 批量获取用户 | `/auth/user/listByIds` | POST | 管理员 |

### 2.2 角色管理 (`/auth/role`)

| # | 接口名称 | URL | 方法 | 权限 |
|---|---------|-----|------|------|
| 14 | 获取用户角色 | `/auth/role/list/{userId}` | GET | 本人或管理员 |
| 15 | 获取全部角色 | `/auth/role/all` | GET | 管理员 |
| 16 | 添加角色 | `/auth/role/add` | POST | 管理员 |
| 17 | 更新角色 | `/auth/role/update` | POST | 管理员 |
| 18 | 删除角色 | `/auth/role/delete` | POST | 管理员 |
| 19 | 分配角色给用户 | `/auth/role/assign` | POST | 管理员 |
| 20 | 移除用户角色 | `/auth/role/unassign` | POST | 管理员 |

### 2.3 权限管理 (`/auth/permission`)

| # | 接口名称 | URL | 方法 | 权限 |
|---|---------|-----|------|------|
| 21 | 获取权限树 | `/auth/permission/tree` | GET | 管理员 |
| 22 | 获取用户权限 | `/auth/permission/user/{userId}` | GET | 本人或管理员 |
| 23 | 添加权限 | `/auth/permission/add` | POST | 管理员 |
| 24 | 更新权限 | `/auth/permission/update` | POST | 管理员 |
| 25 | 删除权限 | `/auth/permission/delete` | POST | 管理员 |

### 2.4 角色权限管理 (`/auth/rolePermission`)

| # | 接口名称 | URL | 方法 | 权限 |
|---|---------|-----|------|------|
| 26 | 分配权限给角色 | `/auth/rolePermission/assign` | POST | 管理员 |
...
获取指定用户的详细信息。当前登录用户可查看自己的信息，管理员可查看任意用户信息。
...
```http
POST /auth/user/changePassword HTTP/1.1
Content-Type: application/json
Authorization: Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9...
...
获取系统中全部角色列表，常用于后台角色选择器或用户角色分配页。
...
| data[].permissionIds | Array<Long> | 已关联权限ID列表 |
...
```http
POST /auth/role/unassign HTTP/1.1
Content-Type: application/json
Authorization: Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9...
...
- 用户不存在时返回 `40401`。
- 角色不存在或用户未分配该角色时返回 `404`。
- 若移除后会导致最后一个管理员失效，则拒绝操作。
