...
-- practice_submission
-- =========================================================
DROP TABLE IF EXISTS `practice_submission`;
CREATE TABLE `practice_submission` (
                                       `id` bigint(20) NOT NULL AUTO_INCREMENT COMMENT '提交ID',
                                       `user_id` bigint(20) NOT NULL COMMENT '用户ID',
                                       `subject_id` bigint(20) NOT NULL COMMENT '题目ID',
                                       `language` varchar(20) NOT NULL COMMENT '语言标识',
                                       `language_id` int(11) NOT NULL COMMENT 'Judge0语言ID',
                                       `code` mediumtext NOT NULL COMMENT '提交代码',
                                       `status` varchar(20) NOT NULL DEFAULT 'PENDING' COMMENT '提交状态：PENDING/RUNNING/AC/WA/TLE/RE/CE/SYSTEM_ERROR',
                                       `pass_case_count` int(11) NOT NULL DEFAULT '0' COMMENT '通过用例数',
                                       `total_case_count` int(11) NOT NULL DEFAULT '0' COMMENT '总用例数',
                                       `execute_time_ms` int(11) DEFAULT NULL COMMENT '执行耗时（毫秒）',
                                       `memory_used_kb` int(11) DEFAULT NULL COMMENT '内存消耗（KB）',
                                       `judge_message` varchar(255) DEFAULT NULL COMMENT '判题摘要',
                                       `stdout_preview` text COMMENT '输出摘要',
                                       `stderr_preview` text COMMENT '错误输出摘要',
                                       `ai_status` varchar(20) DEFAULT NULL COMMENT 'AI分析状态',
                                       `ai_feedback` text COMMENT 'AI分析结果',
                                       `created_time` datetime DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
                                       `update_time` datetime DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
                                       PRIMARY KEY (`id`),
                                       KEY `idx_user_subject` (`user_id`, `subject_id`),
                                       KEY `idx_user_created_time` (`user_id`, `created_time`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='在线判题提交主表';

-- =========================================================
-- practice_submission_case
-- =========================================================
DROP TABLE IF EXISTS `practice_submission_case`;
CREATE TABLE `practice_submission_case` (
                                            `id` bigint(20) NOT NULL AUTO_INCREMENT COMMENT '提交用例ID',
                                            `submission_id` bigint(20) NOT NULL COMMENT '提交ID',
                                            `case_no` int(11) NOT NULL COMMENT '用例序号',
                                            `is_sample` tinyint(1) NOT NULL DEFAULT '0' COMMENT '是否样例用例：1是 0否',
                                            `status` varchar(20) NOT NULL COMMENT '该用例判题状态',
                                            `stdin_text` text COMMENT '输入内容',
                                            `expected_stdout` text COMMENT '预期输出',
                                            `actual_stdout` text COMMENT '实际输出',
                                            `stderr_text` text COMMENT '错误输出',
                                            `execute_time_ms` int(11) DEFAULT NULL COMMENT '执行耗时（毫秒）',
                                            `memory_used_kb` int(11) DEFAULT NULL COMMENT '内存消耗（KB）',
                                            `judge_token` varchar(100) DEFAULT NULL COMMENT 'Judge0提交token',
                                            `judge_message` varchar(255) DEFAULT NULL COMMENT '判题摘要',
                                            `created_time` datetime DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
                                            PRIMARY KEY (`id`),
                                            UNIQUE KEY `uk_submission_case_no` (`submission_id`, `case_no`),
                                            KEY `idx_submission_id` (`submission_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='在线判题提交用例表';
...
CREATE TABLE `subject_info` (
                                ...
                                `subject_type` tinyint(4) DEFAULT NULL COMMENT '题目类型：1单选 2多选 3判断 4简答 5编程',
...
-- subject_code
-- =========================================================
DROP TABLE IF EXISTS `subject_code`;
CREATE TABLE `subject_code` (
                                `id` bigint(20) NOT NULL AUTO_INCREMENT COMMENT '编程题配置ID',
                                `subject_id` bigint(20) NOT NULL COMMENT '题目ID',
                                `judge_mode` varchar(20) NOT NULL DEFAULT 'STANDARD_IO' COMMENT '判题模式',
                                `time_limit_ms` int(11) NOT NULL DEFAULT '1000' COMMENT '时间限制（毫秒）',
                                `memory_limit_kb` int(11) NOT NULL DEFAULT '131072' COMMENT '内存限制（KB）',
                                `supported_languages_json` json NOT NULL COMMENT '支持语言JSON',
                                `template_code_json` json DEFAULT NULL COMMENT '模板代码JSON',
                                ...
                                KEY `idx_subject_id` (`subject_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='编程题配置表';

-- =========================================================
-- subject_code_case
-- =========================================================
DROP TABLE IF EXISTS `subject_code_case`;
CREATE TABLE `subject_code_case` (
                                     `id` bigint(20) NOT NULL AUTO_INCREMENT COMMENT '编程题用例ID',
                                     `subject_id` bigint(20) NOT NULL COMMENT '题目ID',
                                     `case_no` int(11) NOT NULL COMMENT '用例序号',
                                     `stdin_text` text COMMENT '输入内容',
                                     `expected_stdout` text COMMENT '预期输出',
                                     `is_sample` tinyint(1) NOT NULL DEFAULT '0' COMMENT '是否样例用例：1是 0否',
                                     `score` int(11) NOT NULL DEFAULT '1' COMMENT '用例分值',
                                     `created_by` varchar(32) DEFAULT NULL COMMENT '操作人标识',
                                     `created_time` datetime DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
                                     `update_by` varchar(32) DEFAULT NULL COMMENT '更新人标识',
                                     `update_time` datetime DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
                                     ...
                                     `active_subject_case_no` varchar(64)
                                         GENERATED ALWAYS AS (CASE WHEN `is_deleted` = 0 THEN concat(`subject_id`, '_', `case_no`) ELSE NULL END) STORED COMMENT '有效题目用例唯一键',

                                     PRIMARY KEY (`id`),
                                     UNIQUE KEY `uk_active_subject_case_no` (`active_subject_case_no`),
                                     KEY `idx_subject_id` (`subject_id`)
